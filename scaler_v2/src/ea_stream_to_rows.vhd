...
entity stream_to_rows is
   ...
   port ( 
        ...
        o_pix           : out t_dinfo(data(0 to 1)(G_PIX_WIDTH -1 downto 0)) );
   end stream_to_rows;
...
------------------------------------------------------------------------
   ...
   s_iFIFO_data   <= s_oREG1_data when not(s_oREG1_data(0)) else (others => '0');
...
----------------------------------------------------------------
   ...
   o_position     <= std_logic_vector(s_col_cnt);
   o_valid        <= s_oREGF_valid and s_oREG2_valid and or(s_col_cnt);
   o_pix.last     <= s_oREGF_data(1);
   o_pix.eof      <= s_oREGF_data(0);
   o_pix.data(0)  <= s_oREG2_data(G_PIX_WIDTH +2 -1 downto 2);
   o_pix.data(1)  <= s_oREGF_data(G_PIX_WIDTH +2 -1 downto 2);
