...
entity fifo_bank is
   ...
   port (
      ...
      i_valid      : in  std_logic_vector(G_TAP_NO -1 downto 0);
      i_din        : in  t_dinfo_array(0 to G_TAP_NO-1)(data(0 to 0)(G_RD_DWIDTH -1 downto 0));
      --! Read Data Valid: When asserted, this signal indicates
      --! that valid data is available on the output bus (dout).
      -- READ_DATA_WIDTH-bit output:
      -- Read Data: The output data bus is driven when reading the FIFO.
      o_dout       : out t_dinfo_array(0 to G_TAP_NO-1)(data(0 to 0)(G_WR_DWIDTH -1 downto 0));
      o_valid      : out std_logic_vector(G_TAP_NO -1 downto 0);
      i_dready     : in  std_logic_vector(G_TAP_NO -1 downto 0));
   end fifo_bank;

architecture Behavioral of fifo_bank is
   signal s_oFIFO_data   : std_logic_vector((G_RD_DWIDTH +3) * G_TAP_NO -1 downto 0);
   signal s_iFIFO_data   : std_logic_vector((G_WR_DWIDTH +3) * G_TAP_NO -1 downto 0);
   signal s_oFIFO_dvalid : std_logic_vector(G_TAP_NO -1 downto 0);
   signal s_iFIFO_dvalid : std_logic_vector(G_TAP_NO -1 downto 0);
   signal s_oFIFO_ready  : std_logic_vector(G_TAP_NO -1 downto 0);
   signal s_iFIFO_ready  : std_logic_vector(G_TAP_NO -1 downto 0);
begin
...
fifo_bank_gen:
   for gen_var in 0 to G_TAP_NO -1 generate
      ...
      s_iFIFO_data((gen_var +1) * (G_WR_DWIDTH +3) -1 downto gen_var * (G_WR_DWIDTH +3)) <= i_din(gen_var).data(0) & i_din(gen_var).last & i_din(gen_var).eof & i_valid(gen_var);
...
fifo_k_gen:
   for gen_var in 0 to G_TAP_NO -1 generate
      o_dout(gen_var).data(0)  <= s_oFIFO_data((gen_var +1) * (G_RD_DWIDTH +3) -1 downto gen_var * (G_RD_DWIDTH +3) +3);
      o_dout(gen_var).last     <= s_oFIFO_data(gen_var * (G_RD_DWIDTH +3) +2);
      o_dout(gen_var).eof      <= s_oFIFO_data(gen_var * (G_RD_DWIDTH +3) +1);
