...
fifo_bank_gen:
   for gen_var in 0 to G_PHASE_NUM -1 generate
      ...
      s_iFIFO_data((gen_var +1) * (G_WR_DWIDTH +3) -1 downto gen_var * (G_WR_DWIDTH +3)) <= i_din(gen_var / 4)(gen_var mod 4).pix & i_din(gen_var / 4)(gen_var mod 4).last & i_din(gen_var / 4)(gen_var mod 4).sof & i_valid(gen_var);
...
fifo_k_gen:
   for gen_var in 0 to G_PHASE_NUM -1 generate
      o_dout(gen_var / 4)(gen_var mod 4).pix      <= s_oFIFO_data((gen_var +1) * (G_RD_DWIDTH +3) -1 downto gen_var * (G_RD_DWIDTH +3) +3);
      o_dout(gen_var / 4)(gen_var mod 4).last     <= s_oFIFO_data(gen_var * (G_RD_DWIDTH +3) +2);
      o_dout(gen_var / 4)(gen_var mod 4).sof      <= s_oFIFO_data(gen_var * (G_RD_DWIDTH +3) +1);
      o_valid(gen_var)         <= s_oFIFO_data(gen_var * (G_RD_DWIDTH +3));
      o_dready(gen_var)        <= s_oFIFO_ready(gen_var);
   end generate;
